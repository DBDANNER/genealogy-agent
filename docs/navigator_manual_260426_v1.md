# Harvester / Navigator Manual — 260426 v1
...
This version records two validated recurring tasks:
- clearing the queue and reseeding it with a designated starting PID
- redoing a bad harvest when a collapsed or missing page section caused incomplete capture
...
- recover from a bad harvest caused by missing page sections such as collapsed Vitals
...
## Common task: redo a bad harvest caused by collapsed or missing page sections

### Purpose
Recover cleanly when a harvested batch was captured with an important section collapsed or missing, such as Vitals being turned off, causing incomplete raw capture and incomplete `people` records.

### Typical symptom pattern
A future chat should suspect this problem when:
- newly harvested people unexpectedly show null or missing vital fields
- there is repeated “birthplace unknown” or similar evidence across a whole batch
- the captured page display name and tabs are present, but the expected section body is absent from the raw capture
- the user realizes a page section such as Vitals was collapsed during harvesting

### Scope of the recovery
This is a cleanup-and-rerun procedure.

It is **not** primarily an overlay problem.

If the intake was bad, the preferred recovery is to remove the affected bad artifacts and reharvest, rather than patching missing vitals or relationships through commentary overlays.

### Inputs needed
- the affected PID range or exact affected PID list
- the affected time window, if known
- confirmation that the bad batch has **not** yet been mirrored remotely

### Practical identification rule
The easiest practical way to identify the bad batch is:
1. sort affected artifacts by PID, date, and time
2. isolate the bad run window
3. remove the bad artifacts for that run only

### Current validated cleanup boundary
For the affected bad batch, delete the corresponding artifacts from:
- `fs_captures\raw\<PID>\...`
- the equivalent merged layer artifacts
- `people\<PID>.json`

If tree-builder outputs were produced from the bad batch, those outputs should also be discarded and rebuilt.

If no output files were created yet, no builder cleanup is needed.

### Important operational note
Deleting only the `people` files is less safe than deleting all three layers above.

The validated safer pattern is to delete:
1. bad raw captures
2. bad merged artifacts
3. bad `people` records

Then reharvest from a clean state.

### Recommended rerun procedure
After cleanup:
1. reopen FamilySearch with the required page sections expanded
2. reseed the queue to the **first bad PID**
3. reharvest that one person only
4. inspect the new raw JSON and/or the new `people` JSON for that PID
5. confirm that vitals and expected relationship content are now populated
6. only then continue with the rest of the affected batch

### What success looks like for the validation rerun
A successful validation rerun should show that:
- the new raw capture contains the expected Vitals section content in the captured DOM/index
- the new `people` record contains populated fields such as birth/death date and place rather than nulls
- the parsed field metadata points to the new raw capture timestamp and source path

### Validated interpretation note
The top-level convenience fields in a raw capture are not always the best diagnostic signal by themselves.

The real success test is whether the captured DOM/index now includes the expected section content and whether the resulting `people` record is populated correctly from that new capture.

### When to prefer this procedure
Use this procedure when the problem is a bad capture batch caused by page state.

Do **not** elevate immediately to overlays or manual patching unless there is a separate genealogical correction problem after the rerun.
...
- Do not treat a bad capture batch as primarily an overlay problem.
- Do not delete only the `people` layer when a whole bad intake batch can still be identified cleanly.
- Do not rerun the whole batch blindly before validating one corrected reharvest.
- Do not treat builder scripts or other local scripts as part of the reseed contract; this manual is only about Navigator state control and bad-harvest recovery.
...
- harvesting workflow after reseed beyond the validated bad-harvest redo pattern above
...
The current common contract for “redo a bad harvest caused by collapsed sections” is:
- identify the affected PID/time window
- delete the bad artifacts from raw, merged, and `people`
- reseed to the first bad PID
- validate one corrected rerun before continuing the rest of the batch

These are now live validated operational procedures.
