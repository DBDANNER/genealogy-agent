...
At this stage, the constitutional files should be treated as **drafted starter documents**.

They are:
- authoritative enough to use as the current reference layer
- not yet assumed to be installed in `genealogy-agent` unless explicitly confirmed
- expected to improve over time as startup friction reveals gaps

A useful shorthand:
- file index = authoritative v1 when present
- startup/manual files = current working constitutional documents
- folder structure = working proposal, not yet fully frozen
...
1. Startup README / `00_start_here`
...
Special rule:
- If the task involves Shuttle, read the Shuttle manual before guessing any Shuttle contract.
...
5. If needed, consult the file index and fetch the relevant code file(s) from GitHub.
...
Manual path:
- to be maintained in the constitutional file set

Code/index path:
- to be maintained in the file index
...
Manual path:
- see Shuttle manual in the constitutional repo
...
There is currently no separate Codex manual; this overview contains the minimum durable rules.
...
- `genealogy-agent` is planned as the constitutional/reference repo
...
Important note:
GitHub should be treated as a curated reference shelf, not as an unstructured memory blob. New chats should prefer startup index files, manuals, and file indexes with fixed names and fixed paths.
...
If Shuttle knowledge is needed at end-of-session and has not yet been loaded in the current chat, the chat should read the Shuttle manual at that time.
...
Those files should live at fixed paths so that a new chat can fetch them quickly and reliably.
