# End-of-Session Playbook — 260412 v2
...
### 4. Prepare backup artifacts before the final mirror
Before running the final Shuttle mirror, decide whether the session produced constitutional files that should also be backed up as a compact artifact.

Current preferred pattern:
- create a ZIP containing the governing constitutional files that changed during the session
- place that ZIP into a clearly named backup area inside the laptop `ancestry_repo`
- then perform the final Shuttle mirror once

This ordering is preferred because it avoids needing a second mirror pass.

### 5. Mirror the laptop repo to GitHub
After the continuity file, any constitutional updates, and any desired backup ZIP artifacts are already in place, mirror the laptop repo to GitHub.
...
- optional dated ZIP artifact shelf inside `ancestry_repo`
...
3. backup ZIP artifact prepared if desired
4. Shuttle mirror used once to push the completed laptop repo state to GitHub
...
- preserve any desired backup ZIP artifact anyway
...
- optionally create a compact ZIP of governing files changed during the session
- final mirror
...
- prepare any desired ZIP backup artifact before the final mirror
- mirror the laptop repo to GitHub once
